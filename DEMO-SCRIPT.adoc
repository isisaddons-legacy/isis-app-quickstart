...
Will also need to update the seed data for security:
...
public class DomainAppRegularRoleAndPermissions extends AbstractRoleAndPermissionsFixtureScript {
    ...
    protected void execute(final ExecutionContext executionContext) {
        newPackagePermissions(
                ...
                NoteModule.class.getPackage().getName()                 // <1>
                );
    }
}
----
<1> access to functionality contributed by the note module.
...
(Use this technique to hide contributed functionality eg from addon services).
...
to avoid confusion, hide the original `QuickObject#delete()` action:
...
Or, could go further and define a `Deletable` interface:
...
then let `QuickObject`implement this, and refactor the mixin (rename to `Deletable_delete`).
